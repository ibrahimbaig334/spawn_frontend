    ...
    <ol className="m-0 grid list-none gap-1 border-t border-rule p-0" aria-label="Milestone bands">
      {milestones.map((band) => {
        ...
        return (
          <li
            key={band.index}
            className="grid grid-cols-[4.5rem_minmax(0,1fr)_auto_auto] items-center gap-4 border-b border-rule py-2 text-sm max-[48rem]:grid-cols-[4rem_minmax(0,1fr)_auto]"
          ...
          >
            ...
            <span className="hidden font-mono text-[0.68rem] text-ink-muted max-[48rem]:hidden md:block">
              {band.levelLower}–{band.levelUpper}
            </span>
